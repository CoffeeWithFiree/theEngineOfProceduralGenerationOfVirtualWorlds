            ...
            for z in range(Res_z):
                for y in range(Res_y):
                    ...
                    def CreateObj(triangles,biome):
                        ...
                        object_cur = {"vertices": vertices_cur, "triangles": triangles_cur, "position": position_cur, "biome": biome}
                        objects[f"object{i}"] = object_cur
                        i += 1

                    if self.matrix[x, y, z]:
                        if self.matrix[x, y, z] == BiomesType.land:
                            CreateObj(triangles_land, BiomesType.land)
                        elif self.matrix[x, y, z] == BiomesType.sea:
                            CreateObj(triangles_sea, BiomesType.sea)
